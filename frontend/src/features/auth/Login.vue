        ...
        <BaseForm>
            <div class="form-group flex flex-col w-full relative">
                <input
                    ...
                    type="email" name="email" id="email" placeholder="Email" v-model="userInput.email">
            ...
            <div class="form-group flex">
                <input type="checkbox" name="rememberMe" id="rememberMe" class=" w-4">
                <label for="rememberMe" class=" ml-2 text-[#1c1c1c]">Remember me</label>
            </div>
            <router-link class=" text-right text-primary hover:underline">Lupa password?</router-link>
            <BaseButton :id="'loginBtn'" :type="'submit'" :variant="'primary'" @click="submitForm">Masuk</BaseButton>
        </BaseForm>
        <p class=" text-center text-[#1c1c1c] font-normal mt-4">Belum punya akun? Ayo <router-link
                :to="{ name: 'register' }" class=" text-primary hover:underline">
                daftar
            </router-link></p>
    </div>
</template>
