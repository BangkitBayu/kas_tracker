...
const submitForm = async () => {
  ...
  if (result) {
    try {
      ...
      // console.log(error)
      if (error.status === 422) {
        setErrors.value = error.data.errors;
      }
      setErrors.value = {
        form: "Registration failed. Please check your details and try again.",
      };
    }
  }
  ...
  <div id="container">
    ...
    <form @submit.prevent>
      ...
      <div class="form-group">
        <input
          class="p-2.5 w-full rounded-[5px] border border-[#cfcfcf]"
          type="text"
          name="fullname"
          id="fullname"
          placeholder="Nama lengkap"
          v-model="userInput.fullname"
          :class="
            v$.fullname.$error || setErrors.fullname
              ? ' border-red-500'
              : ' border-[#cfcfcf]'
          "
        />
        <p class="info mt-1 ml-1 text-red-500" v-if="v$.fullname.$error">
          {{ v$.fullname.$errors[0].$message }}
        </p>
        <p class="info mt-1 ml-1 text-red-500" v-else-if="setErrors.fullname">
          {{ setErrors.fullname[0] }}
        </p>
      </div>
      <div class="form-group">
        <input
          ...
          :class="
            v$.username.$error || setErrors.username
              ? ' border-red-500'
              : ' border-[#cfcfcf]'
          "
        />
        <p class="info mt-1 ml-1 text-red-500" v-if="v$.username.$error">
          {{ v$.username.$errors[0].$message }}
        </p>
        <p class="info mt-1 ml-1 text-red-500" v-else-if="setErrors.username">
          {{ setErrors.username[0] }}
        </p>
      </div>
      <div class="form-group">
        <input
          ...
          v-model="userInput.school"
          :class="
            v$.school.$error || setErrors.school
              ? ' border-red-500'
              : ' border-[#cfcfcf]'
          "
        />
        <p class="info mt-1 ml-1 text-red-500" v-if="v$.school.$error">
          {{ v$.school.$errors[0].$message }}
        </p>
        <p class="info mt-1 ml-1 text-red-500" v-else-if="setErrors.school">
          {{ setErrors.school[0] }}
        </p>
      </div>
      <div class="form-group">
        ...
        <p class="info mt-1 ml-1 text-red-500" v-else-if="setErrors.email">
          {{ setErrors.email[0] }}
        </p>
      </div>
      <div class="form-group flex flex-col w-full relative">
        ...
        <p class="info mt-1 ml-1 text-red-500" v-else-if="setErrors.password">
          {{ setErrors.password[0] }}
        </p>
      </div>
      <div class="form-group flex flex-col w-full relative">
        <div class="relative flex w-full rounded-[5px] border border-[#cfcfcf]">
        ...
        <p
          class="info mt-1 ml-1 text-red-500"
          v-else-if="setErrors.confirm_password"
        >
          {{ setErrors.confirm_password[0] }}
        </p>
      </div>
      <button class="bg-pr" type="submit" id="registerBtn" @click="submitForm">
        Daftar
      </button>

      <p class="text-center text-[#1c1c1c] font-normal mt-2">
